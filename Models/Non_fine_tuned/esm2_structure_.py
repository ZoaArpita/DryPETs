...
Optional: linking to ESMFold
-----------------------------
This module still knows nothing about 3D structure by design (see module
docstring above) — but ESM2Scorer.fold_top_mutations() is an *opt-in* bridge
to structure_folder.py's ESMFold client, so a scoring run can automatically
produce 3D models for its own top hits without a separate manual step.
It's opt-in (not automatic on every run) because folding is a network call
to a rate-limited free API and a full deep_mutational_scan can produce
thousands of candidates — folding all of them would be slow and likely to
get rate-limited. Score first, fold a shortlist.
"""
...
try:
    from structure_folder import CachingFolder, apply_mutation, extract_plddt
except ImportError:  # pragma: no cover - structure_folder is optional at import time
    CachingFolder = None  # type: ignore[assignment]
...
class ESM2Scorer:
    ...
    def fold_top_mutations(
        self,
        scores: Sequence[MutationScore],
        top_k: int = 10,
        output_dir: str | Path = "structures",
        fold_wild_type: bool = True,
        rate_limit_seconds: float = 1.0,
    ) -> list[dict]:
        """
        Sends the top-K scored mutations (by LLR, descending) directly to
        ESMFold and writes PDB structures to `output_dir`.

        This is the ESM2 -> ESMFold link: it takes whatever `scores` you
        already computed (score_mutations() or deep_mutational_scan()),
        applies each of the top-K mutations to self.sequence, folds each
        resulting mutant sequence, and folds the wild type once for
        comparison. Results are cached on disk (structure_folder.CachingFolder)
        so re-running doesn't re-fold the same sequence twice.

        Note: this folds by LLR alone, with no biological filtering — a
        high-LLR mutation can still land on a catalytic residue. For
        PETase specifically, prefer routing `scores` through
        BiologyAnnotator.top_candidates() first (which excludes catalytic-
        triad hits by default) and pass *that* filtered list in here,
        rather than raw ESM2 output, if you care about active-site safety.

        Args:
            scores: MutationScore list, e.g. from score_mutations() or
                deep_mutational_scan(). Only the top `top_k` by LLR are folded.
            top_k: how many mutant structures to generate.
            output_dir: directory for cached/named PDB files.
            fold_wild_type: also fold self.sequence once, for RMSD/pLDDT
                comparison against the mutants (e.g. in petase_app.py).
            rate_limit_seconds: minimum spacing between ESMFold API calls.

        Returns:
            List of dicts: {mutation, pdb_path, plddt_mean, error}. `error`
            is set (and pdb_path is None) for any mutation that failed to
            fold (e.g. sequence too long for the free API, API downtime) —
            failures don't abort the rest of the batch.
        """
        if CachingFolder is None:
            raise ImportError(
                "structure_folder.py (and its 'requests' dependency) is required for "
                "fold_top_mutations(). Make sure structure_folder.py is on the path and "
                "run: pip install requests"
            )

        folder = CachingFolder(cache_dir=output_dir, rate_limit_seconds=rate_limit_seconds)
        results: list[dict] = []

        if fold_wild_type:
            try:
                wt_pdb_path = folder.cache_dir / "WT.pdb"
                wt_pdb = folder.fold(self.sequence, name="WT")
                plddt = extract_plddt(wt_pdb)
                results.append({
                    "mutation": "WT",
                    "pdb_path": str(wt_pdb_path),
                    "plddt_mean": plddt.mean if plddt else None,
                    "error": None,
                })
            except Exception as e:
                logger.error("Failed to fold wild type: %s", e)
                results.append({"mutation": "WT", "pdb_path": None, "plddt_mean": None, "error": str(e)})

        top_scores = sorted(scores, key=lambda s: s.llr, reverse=True)[:top_k]
        for s in top_scores:
            m = s.mutation
            mut_name = str(m)
            try:
                mutant_sequence = apply_mutation(self.sequence, m.position, m.wt_aa, m.mut_aa)
                pdb_path = folder.cache_dir / f"{mut_name}.pdb"
                mutant_pdb = folder.fold(mutant_sequence, name=mut_name)
                plddt = extract_plddt(mutant_pdb)
                results.append({
                    "mutation": mut_name,
                ...
                logger.error("Failed to fold mutant %s: %s", mut_name, e)
                results.append({"mutation": mut_name, "pdb_path": None, "plddt_mean": None, "error": str(e)})
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="If > 0, send this many top-LLR mutations (plus the wild type) directly to "
             "ESMFold and save PDB structures. 0 (default) = scoring only, no folding.",
    )
    parser.add_argument(
        "--structures-dir",
        type=str,
        default="structures",
        help="Output directory for folded PDB structures (used with --fold-top-k).",
    )
    ...
    if args.fold_top_k > 0:
        logger.info("Folding top %d mutations via ESMFold -> %s", args.fold_top_k, args.structures_dir)
        fold_results = scorer.fold_top_mutations(
            results,
            top_k=args.fold_top_k,
            output_dir=args.structures_dir,
        )
        print(f"\nFolded structures (saved to {args.structures_dir}/):")
        for r in fold_results:
            if r["error"]:
                print(f"  {r['mutation']}\tFAILED: {r['error']}")
            else:
                plddt_str = f"{r['plddt_mean']:.1f}" if r["plddt_mean"] is not None else "N/A"
                print(f"  {r['mutation']}\t{r['pdb_path']}\tmean pLDDT={plddt_str}")
